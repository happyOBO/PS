...
int solution(int n, vector<vector<int>> wires) {
	...
	{
		wires_graph[wires[i][0] - 1].push_back(wires[i][1] - 1);
		wires_graph[wires[i][1] - 1].push_back(wires[i][0] - 1);
	}
	...
	for (int i = 0; i < wires.size(); i++)
	{
		...
		// wires[i][1] 에서 start
		wire_count = bfs(wires[i][1] - 1, n, wires_graph, { wires[i][0] - 1 , wires[i][1] - 1 });
